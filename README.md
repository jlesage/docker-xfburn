...
This project provides a Docker container for [Xfburn](https://docs.xfce.org/apps/xfburn/start).
...
services:
  xfburn:
    image: jlesage/xfburn
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/xfburn:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-xfburn/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/xfburn/tags
...
container, it would proxy all HTTP requests for `xfburn.domain.tld` to
...
server {
	...
	server_name xfburn.domain.tld;

	location / {
	        proxy_pass http://docker-xfburn;
	}
...
server {
	...
	location = /xfburn {return 301 $scheme://$http_host/xfburn/;}
	location /xfburn/ {
		proxy_pass http://docker-xfburn/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /xfburn/websockify {
			proxy_pass http://docker-xfburn/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /xfburn/websockify-audio {
			proxy_pass http://docker-xfburn/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Xfburn integrated into Docker image. |
...
Any desktop notification generated by Xfburn is forwarded to the
...
second, etc). To allow Xfburn to access the first drive, use
this parameter:
